 ...
 * 消息队列使用方法
 * 1 调用MqUtils 里的静态方法，发送消息。tag 确定业务类型
 * 2 创建业务处理消息的观察者，继承observer接口
 * 3 在业务处理类的subscribe 方法，处理具体业务
 * <p>
 * 消息类型请查阅阿里云文档  https://help.aliyun.com/document_detail/96359.html?spm=a2c4g.11186623.3.3.7aa32833XbjDnI
 */
@Slf4j
public class MqUtils {
     ...
     * 延时消息
     *
     * @String key, Message Key 建议消息生产方为每条消息设置尽可能唯一的 Key，以确保相同的 Key 的消息不会超过 64 条，否则消息会漏查。
     * @body: 消息内容;
     * @delayTime:延时时间，微秒，1000 是1秒;
     * @tag: tag;用来判断业务类型
     */
    ...
    public static boolean order(String key, String body, String tag) {
        ...
        // String shardingKey = String.valueOf("supplier");
        try {
            SendResult sendResult = producer.send(msg);
            // 同步发送消息，只要不抛异常就是成功
            if (sendResult != null) {
    ...
    private static Producer getProducer() {
        ...
        // 这里直接用producer 会导致 已赋值  但是未start的现象  所以需要等producer start完再返回
